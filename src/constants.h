...
static std::string rayMarchVertexShader = R"(
    ...
    void main() {
        vec4 clipPos = projectionMatrix * modelViewMatrix * vec4(aPos, 1.0);
        gl_Position = clipPos;
        
        clipPos /= clipPos.w;
        vec4 nearPoint = world_T_clip * vec4(clipPos.x, clipPos.y, -1.0, 1.0);
        vec4 farPoint = world_T_clip * vec4(clipPos.x, clipPos.y, 1.0, 1.0);

        vOrigin = nearPoint.xyz / nearPoint.w;
        vDirection = normalize(farPoint.xyz / farPoint.w - vOrigin);
        uv = vec2(aPos.x/640, aPos.y/360);
    }
)";
...
static std::string viewDependenceNetworkShaderFunctions = R"(
    float indexToPosEnc(vec3 dir, int index){
        ...
        coordinate = coordinate * pow(2.0, float(scaleExponent));
        if ((index - 3) >= 3 * 4) {
    ...
    vec3 evaluateNetwork(vec3 color, vec4 features, vec3 viewDir){
        ...
        for (int i = 0; i < NUM_CHANNELS_ZERO; ++i){
            float input_value = 0.0;
            if (i < 27) {
                input_value = indexToPosEnc(viewDir, i);
            } else if (i < 30){
                input_value =
                    (i % 3 == 0) ? color.r : (
                    (i % 3 == 1) ? color.g : color.b);
            } else {
                input_value = 
                    (i == 30) ? features.r : (
                    (i == 31) ? features.g : (
                    (i == 32) ? features.b : features.a));
            }

            if (abs(input_value) < 0.1/ 255.0) continue;
            for (int j = 0; j < NUM_CHANNELS_ONE; ++j){
                intermediate_one[j] += input_value * texelFetch(weightsZero, ivec2(i, j), 0).x;
            }
        }
        ...
        for (int i = 0; i < NUM_CHANNELS_ONE; ++i){
            if (intermediate_one[i] <= 0.0) continue;
            for (int j = 0; j < NUM_CHANNELS_TWO; ++j){
                intermediate_two[j] += intermediate_one[i] * texelFetch(weightsOne, ivec2(i, j), 0).x;
            }
        }
        ...
        for (int i = 0; i < NUM_CHANNELS_TWO; ++i){
            if (intermediate_two[i] <= 0.0) continue;
            for (int j = 0; j < NUM_CHANNELS_THREE; ++j){
                result[j] += intermediate_two[i] * texelFetch(weightsTwo, ivec2(i, j), 0).x;
            }
        }
...
static std::string rayMarchFragmentShaderBody = R"(
    ...
    vec3 convertDirectionToNDC(vec3 origin, vec3 direction) {
        ...
        float d0 = 1.0 / (W / (2.0 * focal)) * direction.x / direction.z;
        float d1 = -1.0 / (H / (2.0 * focal)) * (direction.y / direction.z - origin.y / origin.z);
